...
// (옵션, 기본 on) YouTube 자동완성으로 "실제 유튜브 검색 수요"를 근사해 상위 후보를 재정렬합니다.
// - validate=yt|off
// - validateTop=0~200 (기본 60)
const validate = String(reqQuery.validate ?? reqQuery.ytValidate ?? process.env.INTEREST_VALIDATE ?? 'yt').toLowerCase();
const validateTop = clampInt(reqQuery.validateTop ?? process.env.INTEREST_VALIDATE_TOP ?? 60, 0, 200);
debug.steps.validate = { validate, validateTop };

if (validate !== 'off' && validateTop > 0) {
  const cand = entries.slice(0, Math.min(entries.length, validateTop));
  let ci = 0;
  const conc = 6;
  ...
  async function vWorker(){
    while (ci < cand.length){
      const i = ci++;
      const term = cand[i].term;
      try{
        const sugg = await fetchYouTubeSuggest(term, { geo, hl });
        cand[i].ytDemand = scoreYtDemand(term, sugg);
        cand[i].ytSample = (Array.isArray(sugg) ? sugg : []).slice(0, 8);
      } catch (e){
        debug.errors.push({ step: 'ytValidate', term, error: e?.message || String(e) });
        cand[i].ytDemand = 0;
        cand[i].ytSample = [];
      }
    }
  ...
  // raw score에 수요 보너스(최대 +70%) 적용
  for (const x of cand){
    const bonus = (Number(x.ytDemand) || 0) / 100;
    x.scoreAdj = x.scoreRaw * (1 + bonus * 0.7);
  }
  for (let i = cand.length; i < entries.length; i++){
    entries[i].scoreAdj = entries[i].scoreRaw;
  }

  entries.sort((a, b) => (b.scoreAdj - a.scoreAdj) || (b.scoreRaw - a.scoreRaw));
  debug.steps.validate.validated = cand.length;
} else {
  for (const x of entries) x.scoreAdj = x.scoreRaw;
}
...
function scoreYtDemand(query, suggestions){
  const q = String(query || '').toLowerCase().trim();
  const list = Array.isArray(suggestions) ? suggestions : [];
  if (!q || list.length === 0) return 0;

  const norm = list.map(s => String(s || '').toLowerCase().trim()).filter(Boolean);
  const count = Math.min(12, norm.length);
  const contains = norm.filter(s => s.includes(q)).length;

  const tails = new Set();
  for (const s of norm){
    let t = s.startsWith(q) ? s.slice(q.length).trim() : s;
    t = t.replace(/^[\-\–\—\:\|\·\.,]+/g, '').trim();
    const first = (t.split(/\s+/)[0] || '').slice(0, 12);
    if (first) tails.add(first);
  }
  const diversity = Math.min(12, tails.size);

  // 간단 근사: 자동완성 후보 수 + 포함도 + 꼬리 다양성
  const sc = Math.round(Math.min(100, count * 6 + contains * 2 + diversity * 4));
  return clampInt(sc, 0, 100);
}
